...
function Dashboard({ logout }) {
    ...
    const [totalProfitRatio, setTotalProfitRatio] = useState(0)
    ...
    function getLastSellRate (val) {
        ...
        if (closedTrades.length > 0) {
            ...
            let latestRate = sortedTrades[0]['limit'] || 'N/A' 
            return latestRate
        } else {
            return 'N/A'
        }
    ...
    let getTradeBots = async function () {
        ...
        if (token) {
            try {
                ...
                setTotalProfitRatio(totalTradesRatio)
                                    ...
                                    <div class="flex items-center space-x-2">
                                        <a class="rounded-full focus:outline-none focus:ring-2  focus:bg-indigo-50 focus:ring-indigo-800" href=" javascript:void(0)">
                                            <div class="py-2 px-8 bg-indigo-50 text-indigo-700 rounded-full">
                                                <p>Total Profits: <span class="text-black mx-1">${totalProfit.toFixed(2)}</span><span className={"mx-1 text-gray-500"}>({(totalProfitRatio * 100)}%)</span></p>
                                            </div>
                                        </a>
                                        <a class="rounded-full focus:outline-none focus:ring-2 focus:bg-indigo-50 focus:ring-indigo-800" href="javascript:void(0)">
                                            <div class="py-2 px-8 bg-indigo-50 text-indigo-700 rounded-full">
                                                    ...
                                                    <tr key={index} tabindex="0" class="mb-3 focus:outline-none h-16 border border-gray-100 rounded">
                                                        ...
                                                        <td class="pl-5">
                                                            <div class="flex items-center">
                                                                ...
                                                                <p class="text-sm leading-none text-gray-600 ml-2" ><span class="text-black">${(val.trades.reduce( (accumulator, _val ) => { return accumulator + Number(_val['profit_amount'] || 0) }, 0) || 0).toFixed(2)}</span></p>
                                                            </div>
                                                        </td>
